"""
Task 1 -- NO2-ONLY model (meteo removed).

Same pipeline as task1_model_improved.py, but the feature set is stripped to
NO2-derived inputs only:
  * ground NO2 history  (lags 1/2/3/7/14, rolling 7/14)
  * satellite NO2       (s5p mean, lag 1/7, rolling 7, pixel count)
  * calendar            (weekday/weekend/month/day-of-year, holidays, August)
NO temperature, humidity, wind, pressure or rain.

Everything else is identical (log target, per-year features that never cross the
winter gap, real labels only, train 2022-2024 / test 2025) so the result is
directly comparable to the full model -- the difference IS the meteo contribution.
"""
...
def engineer(df):
    ...
    # label (never interpolated) + a filled copy used ONLY for lag features
    ...
    # satellite: short interpolation within year, then year-median fill
    ...
    # lags / rolling within each year (never across the winter gap)
    def py(col, fn):
...
def main():
    ...
    print(f"  train 2022-2024: {len(train)}   test 2025: {len(test)}  (NO2-only, no meteo)\n")
    ...
    if HAVE_XGB:
        ...
        print("\n  Top 10 features (XGBoost, NO2-only):")
        for k, v in imp.head(10).items():
            print(f"    {k:22s} {v:.3f}")
    else:
        print("\n(xgboost not installed -> run:  pip install xgboost  to enable it)")

    print("\nThis is the NO2-only model. Compare its R2/RMSE to the full model:")
    print("if they are close, meteo was adding little and NO2 history + satellite carry the signal.")
